...
| 分支 | 负责人 | 用途 | 合并权限 |
|------|--------|------|----------|
| main | B（你） | 稳定可演示版，代码最终汇总到这里 | 只有 B |
| coordinator | B（你） | 骨架/LLM封装/FastAPI/Coordinator/Web/集成/答辩模拟 | B → main |
| agent/planner | 队友A | Planner Agent：课程→子任务JSON | B → main |
| agent/timeline-qa | 队友C | Timeline + QA责任矩阵 + 可解释性 | B → main |
...
| feat/timeline: | feat/timeline: 关键路径标红 |
