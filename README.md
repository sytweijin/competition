...
项目信息（名称 / 成员 / 截止日 / 每日工时）
     ...
     v
+-----------+   确认    +-----------+      +------------+      +-----------+
|  Planner  |---草案--->|  Matcher  |----->|  Timeline  |----->|  Reporter |
+-----------+          +-----------+      +------------+      +-----------+
  知识库校准工时        技能匹配+分工        CPM 关键路径          项目报告
  1-8 弹性子任务        负责人/协作者        倒排日期+浮动          Markdown
  去重/去环/清依赖      负载均衡             日工时折算             Word / PDF
                                                                        |
                     +----------------------------------------------------+
                     |
                     v
              用户可随时增删改任务、拖拽调整分工、切换任务状态
              -> 重算 Timeline + Matcher + Reporter
                     |
                     v
              Project Service 统一编排（generate_draft / confirm_draft /
