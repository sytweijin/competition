...
def test_draft_editor_uses_handle_only_dragging_and_always_analyzes_files():
    ...
    assert 'id="taskEditor"' in html
    assert '<article class="task-edit-card" data-id="' in js
    draggable_card = """class="task-edit-card" data-id="'+esc(task.id)+'" draggable="""
    assert draggable_card not in js
    assert 'class="drag-handle" draggable="true"' in js
