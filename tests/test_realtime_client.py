...
@pytest.mark.asyncio
async def test_voice_requirement_local_returns_verbatim_transcript(monkeypatch):
    """本地答辩语音需求：返回逐字转写原话，而不是模型的整理或回答。"""
    import app.services.media_analysis as media
    import app.web.routers.realtime as realtime_router
...
@pytest.mark.asyncio
async def test_voice_requirement_cloud_returns_understanding(monkeypatch):
    """云端答辩语音需求：保持原有"整理需求要点"行为（转写 → 理解）。"""
    import app.services.media_analysis as media
    import app.services.omni_chat as omni_chat
    import app.services.realtime_client as rt
    import app.web.routers.realtime as realtime_router
