...
本地输出跑偏成建议。
3. **修改后：** 接口改为复用配置页同款逐字转写链路 `audio_transcribe_text`
（本地 12 秒分片 + 乱码/客套守卫 + 重试），两端统一返回用户原话；
删除"整理要点"指令常量；转写失败返回 502 并给出本地/云端区分提示。
4. **为什么这样改：** 用户需要的是可粘贴进答辩稿/评委关注点的原话；
转写链路已在配置页语音需求经过验证，比"让模型理解整理"更符合预期，
也更抗本地模型把指令当对话的跑偏。
5. **收益：** ① 答辩语音需求返回逐字转写原话；② 与配置页语音需求行为一致；
   ③ 本地跑偏时明确报错而不是返回建议。
...
（复用配置页音频转写链路，本地+云端一致）；全量测试 373 → 385 passed；
