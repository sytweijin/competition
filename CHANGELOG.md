...
1. 新增 `app/knowledge/duration_examples.json`，覆盖确认提交、资料整理、调研、文案、报告、PPT、排版、视频、开发、测试和演练等常见课程任务。
2. 新增轻量检索与校准服务：项目输入先检索相关案例提供给 Planner，生成后的任务再统一计算建议值、合理范围、依据和可信度。
3. 删除通用兜底中“按团队总产能等比缩放任务工时”的逻辑；成员产能只用于后续超载检查。
4. 任务卡新增参考范围与可信度提示，同时保留人工编辑入口。
5. 新增回归测试，覆盖短任务降估、成果规模差异、估时解释字段，以及团队产能变化不影响同一任务工时。
