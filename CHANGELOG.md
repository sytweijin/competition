...
### 同步修改：系统架构图对齐（2026-08-30 追加）

**定位：** 按评审意见重绘 README 与项目说明书中的系统架构图，统一框体宽度、连接符列对齐，消除线段错位。本次为文档调整，不涉及代码。

1. **问题：** 项目说明书 3.1 架构图顶部与中部框的内容行比边框宽（48/49–52 对 46/48），README 架构图整张未对齐，存在多余的边框行且 `┬/│/▼` 连接列不一致，评审阅读时观感杂乱。
2. **修改前：** 两张图均为手工排版的混排 ASCII 图，框体不呈矩形，三处连接（前端→后端、后端→昇腾、后端→云端）的竖线与箭头未在同一列。
3. **修改后：** 用等宽（CJK 双宽感知）方式重绘两张架构图：前端框宽 46、后端框宽 48、底部两框各宽 22 且箭头居中；三组连接列分别对齐到 22、11、35；程序化校验框体矩形性与连接符列位置后写入文档。
4. **为什么这样改：** 架构图是评审最先看到的工程结构信息，框体与连接线的对齐直接影响专业感；等宽排版可在任意等宽字体下保持对齐。
5. **收益：** ① 两张架构图框体矩形、连接线逐列对齐；② 底部双框等宽、箭头居中，视觉平衡；③ 项目说明书与 README 的架构图风格统一。
