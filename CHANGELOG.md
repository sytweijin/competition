...
### 同步修改：文字答辩模拟点评与追问脱离用户回答（2026-08-31 追加）

**定位：** 修复文字答辩模拟（`/api/interview/chat`）评委点评与追问与用户回答无关的问题：回答被当作"当前问题"发送、初始出题指令残留诱导模型从材料另起问题，系统提示词也未像语音链路那样强制"基于回答点评、不到位同维度追问"。本次为代码与提示词修复，不新增版本号。

1. **问题：** 用户实测文字答辩多轮，评委的下一个问题与用户刚给出的回答毫无关联（"我答我的，他问他的"）；语音/视频答辩（`interview-turn`）正常，文字链路异常。
2. **修改前：** `chat_turn` 在回答模式下直接把用户回答作为最后一条 user 消息；合规模式下 `_realtime_text` 将历史摊平并把最后一条标记为"【当前问题】"，8B 模型把"回答"误读为"提问"，且摊平上下文残留"请开始模拟答辩，针对材料提第一个问题"的初始指令，进一步诱导其从材料另出题；`INTERVIEW_CHAT_SYSTEM` 未像语音链路那样强制"点评是否正面回答/是否回避遗漏、不到位同维度追问"。
3. **修改后：** 回答模式最后一条消息改为显式指令："请针对我刚刚的回答给出点评，再提出下一个问题。我刚刚的回答：<回答>"，即使经摊平后模型也能明确区分"这是回答、要对它点评追问"；`INTERVIEW_CHAT_SYSTEM` 开头改为强制规则：第一段点评（是否正面回答/回避/遗漏/完整/依据到位），第二段下一问（不到位同维度追问、到位才换维度），禁止忽略回答另起无关问题，禁止复述上一轮点评与问题；新增回归测试断言回答以"我刚刚的回答"指令发送且包含"点评"。
4. **为什么这样改：** 8B 模型对隐式上下文不敏感，需要把"待处理对象"和"要做的事"显式写进最后一条消息；文字与语音两条答辩链路应保持同等的"基于回答点评与追问"强度。
5. **收益：** ① 文字答辩评委点评与下一问基于用户回答；② 回答不到位时同维度追问、到位才换维度；③ 文字/语音/视频三条答辩链路行为一致；④ 新增回归测试，全量 402 passed。

**涉及文件：** `app/agents/interview_sim.py`、`app/llm/prompts.py`、`tests/test_interview_chat.py`、`CHANGELOG.md`。

### 同步修改：全量测试数 401 → 402 与提交材料同步（2026-08-31 追加）

**定位：** 文字答辩模拟修复新增一条回归测试后全量测试数由 401 变为 402，同步更新提交材料中写死的测试数。本次为文档调整，不涉及代码。

1. **问题：** 全量测试 402 passed，但项目说明书（摘要、5.1、8.4）、README（验证命令注释、项目结构注释）、复现文档（验证命令注释、实测结果表）、AGENTS.md（前端规则基线）仍写 401，PPT 工程验证页仍写 401，评委核对时会出现不一致。
2. **修改前：** 上述材料统一写"401 项自动化测试 / 401 passed"。
3. **修改后：** 项目说明书、README、复现文档、AGENTS.md 全部改为 402，实测日期统一为 2026-08-31；PPT 由用户自行更新。
4. **为什么这样改：** 提交材料中的测试数必须与实测一致，评审按图索骥时不能出现数字对不上。
5. **收益：** ① 各提交材料测试数口径统一为 402；② 说明书摘要"三项硬证据"与实测一致；③ PPT 由用户侧同步。

**涉及文件：** `docs/项目说明书.md`、`README.md`、`docs/复现文档.md`、`AGENTS.md`、`CHANGELOG.md`。
